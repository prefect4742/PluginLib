...
object Preconditions {
     ...
     * be converted to a string using [String.format]
     * @param messageArgs arguments for `messageTemplate`
     * @throws IllegalArgumentException if `expression` is false
     */
    fun checkArgument(
        expression: Boolean,
        ...
        if (!expression) {
            throw IllegalArgumentException(String.format(messageTemplate, *messageArgs))
        }
    }
    ...
    fun <T> checkNotNull(
        ...
        if (reference == null) {
            throw NullPointerException(String.format(messageTemplate, *messageArgs))
        }
        return reference
    }
    ...
    fun checkArgumentInRange(
        ...
        when {
            java.lang.Float.isNaN(value) -> throw IllegalArgumentException("$valueName must not be NaN")
            value < lower -> throw IllegalArgumentException(
                String.format(
                    "%s is out of range of [%f, %f] (too low)", valueName, lower, upper
                )
            )
            value > upper -> throw IllegalArgumentException(
                String.format(
    ...
    fun <C : Collection<T>, T> checkCollectionElementsNotNull(
        ...
        for ((ctr, elem) in value.withIndex()) {
            if (elem == null) {
                throw NullPointerException(
    ...
    fun checkArrayElementsInRange(
        ...
        for (i in value.indices) {
            ...
            when {
                java.lang.Float.isNaN(v) -> throw IllegalArgumentException("$valueName[$i] must not be NaN")
                v < lower -> throw IllegalArgumentException(
                    String.format(
                        "%s[%d] is out of range of [%f, %f] (too low)",
                        valueName, i, lower, upper
                    )
                )
                v > upper -> throw IllegalArgumentException(
                    String.format(
     ...
     */
    fun checkArrayElementsInRange(
        value: IntArray, lower: Int, upper: Int,
        valueName: String
    ): IntArray {
        checkNotNull(value, "$valueName must not be null")
